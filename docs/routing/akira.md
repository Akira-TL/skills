...
Akira 管理的 Skill 只从 Catalog 登记的远端 GitHub source 拉取到 `~/.agents/sources/`，再以软链接注册到机器级 `~/.agents/skills/`。安装逻辑由 Lattice 自带的 `scripts/skills.py` 实现，不依赖第三方 Skill package manager。ForgeRelay、Claude Code、Codex 或其他执行器自己的 Skill 目录只作为执行器视图，由执行器自行软链接到 `~/.agents/skills/`；Akira 安装器不管理这些目录。
