...
Agent 可以打开登录入口、选择机构登录、定位验证码区域并完成机械步骤。密码、验证码、二次认证以及用户要求亲自处理的敏感授权交给用户在可见浏览器中完成。

用户完成后，Agent 应先验证浏览器已经进入预期认证状态，再继续抓取、解析资源或填写页面。登录态由浏览器/Profile 自己保存，不把 Cookie、密码或 token 写进项目文件。
...
```bash
npx skills add . --skill browser-access --agent '*' -g -y
```

或从 GitHub：

```bash
npx skills add Akira-TL/skills --skill browser-access --agent '*' -g -y
```
