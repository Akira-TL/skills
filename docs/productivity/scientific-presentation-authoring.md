...
中文页面文案完成后，Skill 必须调用显式依赖 `humanizer-zh` 做语言终检，删除空泛的意义句、公式化排比、宣传式动词和抽象总结，同时保持研究事实、统计结果、术语和证据强度不变。该依赖不随本 Skill 预装：当前会话和机器级注册表均缺失时，由 `akira` Router 按 `EXTERNAL-SOURCES.md` 登记的 `op7418/Humanizer-zh` 来源说明用途并取得用户明确同意后按需安装；若用户不允许安装，应明确说明 Humanizer 终检未执行。
