...
先检查当前会话真实可用能力；当前会话不可用时，再判断当前执行器是否采用 Akira 机器级注册表 `~/.agents/skills/`。采用共享注册表的执行器才检查并复用其中的同名受管 Skill；使用独立 Skill store 的执行器只检查自己的可用来源，不得为了兼容自动建立到 `~/.agents/skills/` 的跨 store 适配链接。当前执行器允许的来源中仍缺少能力时，再按 Catalog 确定远端 GitHub source 与最小 Skill 集合。
...
完整产品仍然按真实需求才安装。采用 Akira 共享注册表的执行器可按 `INSTALLATION.md` 把远端 GitHub source 更新到共享 checkout，并把选中的 Skill 注册到机器级 `~/.agents/skills/`；使用独立 Skill store 的执行器则只复用 Catalog 的来源与最小集合信息，实际安装和暴露方式完全遵守执行器自己的机制。跨域需求出现时再增加第二个产品，不因“可能以后用到”预装。
...
当前任务为什么需要
计划安装的 Skill 或完整产品族
安装范围：当前执行器实际采用的 Skill store
当前发布状态
```

用户明确同意后才能执行。准确 GitHub source 与 Skill 集合从 Catalog 读取，不凭记忆拼仓库名或 Skill 名称。只有当前执行器明确采用 Akira 共享注册表时，才读取 `INSTALLATION.md` 并核验机器级 `~/.agents/skills/<name>` 指向共享 Git checkout；执行器使用独立 Skill store 时，不得调用 Akira 安装器替它建立适配链接，而应遵守该执行器自己的安装契约。若目标处于 `remote-pending`、`planned` 或其他不可直接取得状态，保持 blocker，不把未来仓库伪装成已发布来源。
