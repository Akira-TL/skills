...
本 Skill 当前处于 `in-progress`。`research-db` 已实现 schema migration、Discovery Search Run / Candidate 队列、Candidate identity reconciliation、论文 acquisition、Pass 1 Reconstruction、Pass 2 Critical Audit、FTS/evidence 检索、跨论文 relation、Discovery closure，以及项目自身 Hypothesis Set / Research Design / Dataset / Analysis Run / Analysis Amendment / Project Observation 的最小 provenance 和最终 `validate --completion` 门禁。搜索、候选发现、canonical artifact、论文知识单元、项目假设/设计身份、项目数据/分析结果、批判问题、关系与 change log 均由同一项目数据库记录必要语义和 pointer；完整 Hypothesis/Design 科研正文、原始论文、数据、代码和结果文件仍保持为独立 artifact。Evidence Synthesis 规则见 [`references/RESEARCH-SYNTHESIS.md`](references/RESEARCH-SYNTHESIS.md)。Agent 通过脚本维护数据库，不把直接散写 SQL 作为正常科研工作流。
