...
从 schema v23 起，`user_reading_events` 保存用户本人对人类论文 sidecar 的追加式阅读确认历史，action 为 `confirmed | revoked | invalidated`。它与 Agent 的 `reading_status` / `critical_status` 完全分离。每次事件记录 `sidecar_path` 与归一化后的 Git content OID：计算前把顶部/结尾复选框都视为未勾选，并把 `<!-- akira:user-notes:start --> ... <!-- /akira:user-notes:end -->` 之间的用户专属笔记归一化为空区块。因而点击复选框或用户继续补写自己的笔记不会改变 Agent 阅读正文的版本身份；Agent 生成区域发生变化才会使旧确认不再适用于当前 content OID。残留的旧 `[x]` 会在同步时被 fail-closed 地标记 invalidated 并清空，不能替用户确认新版本。人类 sidecar 的新格式使用 `<!-- akira:literature-note:v1 -->` 标记；completion 会机械核验唯一一级标题、固定书目信息表、固定二级标题顺序、用户笔记边界与双确认框，但不评价字数、实验数或图表数。没有 v1 标记的历史 sidecar 继续按旧格式兼容；出现用户笔记区等现代结构后必须完整采用 v1，避免半新半旧。
