...
先判断 Observation 识别的是某一时间点/条件下的**状态差异或关联**，还是能够支持随时间、暴露或过程发生的**变化**。只有数据结构和设计能够识别变化方向、时间顺序或过程轨迹时，才把静态状态升级为过程性表述。

例如“更相似”“距离更小”“组成相关”是状态描述；“收敛（convergence）”“发散（divergence）”“获得（acquisition）”“定殖（colonization）”“传播（transmission）”“适应（adaptation）”“恢复（recovery）”“进展（progression）”等术语通常隐含变化、历史或方向。横断面数据只观察到某一时点的相似性时，应报告相似性、差异、关联或对应关系，而不是把它改写为发生了收敛、获得或传播。若领域中存在对某个过程术语的特殊操作定义，只有在引用该定义并满足其识别条件时才使用。

### 一致性与独立重复验证

同一 Dataset、同一 sampling frame、同一 cohort 中的不同材料、亚组、outcome、assay 或分析规格，首先用于判断**内部一致性、异质性或敏感性**。它们共享样本来源、研究实施或未测量因素时，不自动构成独立重复验证（independent replication）。

只有对目标 Claim 真正提供新的独立证据单位时才使用“独立重复验证”或等价表述；否则明确写成“在不同材料中方向一致”“在多个分析规格下稳定”“跨亚组观察到相似模式”等与实际设计相符的描述。不同地点是否构成独立重复单位取决于 sampling design、推断单位及依赖结构，不能只因地点名称不同就默认独立。
